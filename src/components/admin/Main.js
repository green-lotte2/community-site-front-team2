...
const Main = () => {
  ...
  const articleDetailHandler = (index) => {
    ...
    axios
      .get(
        url.backendUrl +
          "/admin/searchArticleCause?no=" +
          serverData.dtoList[index].no
      )
      .then((response) => {
  ...
  const stopArticleHandler = (userId) => {
    axios
      .get(`${url.backendUrl}/admin/stopArticle?no=` + userId)
      .then((response) => {
        alert("배공개 처리되었습니다.");
  ...
  const unStopArticleHandler = (userId) => {
    axios
      .get(`${url.backendUrl}/admin/unStopArticle?no=` + userId)
      .then((response) => {
        alert("배공개 처리되었습니다.");
        setArticleModal(false);
  ...
  useEffect(() => {
    axios
      .post(`${url.backendUrl}/admin`, initState)
      .then((response) => {
        setServerData(response.data);
      })
      .catch((err) => {
  ...
  const stopUserHandler = (userId) => {
    axios
      .get(`${url.backendUrl}/admin/stopUser?uid=` + userId)
      .then((response) => {
        console.log(response.data.dtoList);
        alert("정지 되었습니다.");
        setOpenModal(false);
  ...
  const unStopUserHandler = (userId) => {
    axios
      .get(`${url.backendUrl}/admin/unStopUser?uid=` + userId)
      .then((response) => {
        alert("정지 해제되었습니다.");
  ...
  const [state, setState] = useState(
    new Array(serverData.dtoList.length).fill(false)
  );
  useEffect(() => {
  ...
  const faqSubmitHandler = () => {
    ...
    if (window.confirm("제출하시겠습니까?")) {
      axios.post(`${url.backendUrl}/admin/faqInsert`, faq).then((rep) => {
  ...
  const delteFaqHandler = (no) => {
    if (window.confirm("삭제하시겠습니까?")) {
      axios.get(`${url.backendUrl}/admin/deleteFaq?no=` + no).then((rep) => {
        ...
        const updatedDtoList = serverData.dtoList.filter(
          (item) => item.no !== newInt
        );
      ...
      <div className="Board">
        <div className="freeBoard">
          ...
          <div className="cate">
            <Link
              to="/admin?cate=user"
              className={cate === "user" ? "backGround" : ""}
            >
              악성 유저
            </Link>
            <Link
              to="/admin?cate=article"
              className={cate === "article" ? "backGround" : ""}
            >
              악성 게시물
            </Link>
            <Link
                ...
                {serverData.dtoList.map((user, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td style={{ width: "30%" }}>
                        <span style={{ marginLeft: "10px" }} id="status">
                          {user.reportStart == null ? (
                            <span>미정지</span>
                          ) : (
                            <span>
                              정지: {user.reportStart}~{user.reportEnd}
                            </span>
                          )}
                        </span>
                      </td>
                      <td style={{ width: "15%" }}>
                        <p onClick={() => memberDetailHandler(index)}>
                          <img
                            src="/images/magnifier_50.png"
                            style={{ width: "28px" }}
                          />
                        </p>
                      </td>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th style={{ width: "15%" }}>
                    받은신고횟수
                    <span onClick={articleOrderHandler}>
                      <img
                        src="/images/upDown.png"
                        style={{
                          width: "17px",
                ...
                {serverData.dtoList.map((article, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td style={{ width: "10%" }}>
                        {article.status === 1 ? <>비공개</> : <>공개</>}
                      </td>
                      <td style={{ width: "20%" }}> {article.writer} </td>
                      <td style={{ width: "15%" }}>
                        <p onClick={() => articleDetailHandler(index)}>
                          <img
                            src="/images/magnifier_50.png"
                            style={{ width: "28px" }}
                          />
                ...
                {serverData.dtoList.map((article, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td style={{ width: "30%" }}>
                        <Link to={`/admin/view?no=${article.qnaPk}`}>
                          {article.title}
                        </Link>
                      </td>
                      <td style={{ width: "20%" }}> {article.writer} </td>
          ...
          {initState.cate === "faq" ? (
            <>
              <div>
                <Link to="/admin?cate=faq&type=회원/탈퇴" className="myCate">
                  회원/탈퇴
                </Link>
                <Link to="/admin?cate=faq&type=채팅" className="myCate">
                  채팅
                </Link>
                <Link to="/admin?cate=faq&type=프로젝트" className="myCate">
                  프로젝트
                </Link>
                <Link to="/admin?cate=faq&type=캘린더" className="myCate">
                  캘린더
                </Link>
                <Link to="/admin?cate=faq&type=페이지" className="myCate">
                  페이지
                </Link>
                <Link to="/admin?cate=faq&type=기타" className="myCate">
                  기타
                </Link>
              </div>
              <br />
              ...
              <div>
                {serverData.dtoList.map((data, index) => {
                  return (
                    <>
                      <p
                        className="qnaTitle"
                        onClick={() => yameAcoHandler(index)}
                      >
                        Q {data.title}
                        <span
                          className="deleteGahee"
                          onClick={() => delteFaqHandler(data.no)}
                ...
                <p
                  className="addBox"
                  style={{ display: bubutonShow ? "block" : "none" }}
                ...
                <div
                  style={{
                    ...
                    padding: "14px",
                    display: !bubutonShow ? "block" : "none",
                  }}
                >
                  <input
                    type="text"
        ...
        >
          <h2>
            멤버 상세 정보
            <button
              style={{ marginLeft: "250px", border: "none", fontSize: "20px" }}
          ...
          <div style={{ fontSize: "20px", padding: "5px" }}>
            {selectedUser ? (
              <>
                <p>
                  아이디 :
                  <span style={{ marginLeft: "10px" }} id="id">
                    {selectedUser.uid}
                  </span>
                </p>
                <p>
                  닉네임 :
                  <span style={{ marginLeft: "10px" }} id="nick">
                    {selectedUser.nick}
                  </span>
                </p>
                <p>
                  이름 :
                  <span style={{ marginLeft: "10px" }} id="name">
                    {selectedUser.name}
                  </span>
                </p>
                <p>
                  상태 :
                  <span style={{ marginLeft: "10px" }} id="status">
                    {selectedUser.reportStart == null ? (
                      <span>미정지</span>
                    ) : (
                      <span>
                        정지: {selectedUser.reportStart}~
                        {selectedUser.reportEnd}
                      </span>
                    )}
                  </span>
                </p>
                <p>
                  신고횟수 :
                  <span style={{ marginLeft: "10px" }} id="report">
                    {selectedUser.report}
                  </span>
                </p>
                <p id="reason">신고사유</p>
                ...
                {selectedUser.reportStart == null ? (
                  <button
                    ...
                    style={{ marginLeft: "41%" }}
                    onClick={() => stopUserHandler(selectedUser.uid)}
                  >
                    정지하기
                  </button>
                ) : (
                  <button
                    ...
                    className="chatButtonp"
                    style={{ marginLeft: "41%" }}
                    onClick={() => unStopUserHandler(selectedUser.uid)}
                  >
                    정지해제
                  </button>
                )}
              ...
              <>
                ...
                {selectedArticle.status === 0 ? (
                  <button
                    ...
                    className="chatButtonp"
                    style={{ marginLeft: "41%" }}
                    onClick={() => stopArticleHandler(selectedArticle.no)}
                  >
                    비공개처리
                  </button>
                ) : (
                  <button
                    ...
                    className="chatButtonp"
                    style={{ marginLeft: "41%" }}
                    onClick={() => unStopArticleHandler(selectedArticle.no)}
                  >
                    공개처리
                  </button>
                )}
