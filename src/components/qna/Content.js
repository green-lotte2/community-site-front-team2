...
const Content = () => {
  ...
  useEffect(() => {
    axios
      .post(`${url.backendUrl}/qnaContent`, initState)
      .then((response) => {
  ...
  return (
    <div className="Board">
      <div className="freeBoard">
        <div className="commu_title" style={{ backgroundColor: "#ff6b5a" }} s>
          <div className="ctext">
            <h2>고객센터 페이지</h2>
              ...
              {serverData.dtoList.map((qna, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td style={{ width: "25%" }}>
                      {" "}
                      <Link to={"/qna/view?cate=qna&no=" + qna.qnaPk}>
                        {qna.title}
                      </Link>{" "}
                    </td>
                    ...
                    <td
                      className={
                        qna.status === "답변완료" ? "completed" : "in-progress"
                      }
                    >
                      {qna.status}
                    </td>
      ...
      {initState.cate === "faq" ? (
        <>
          <div className="cate">
            <Link to="/qna?cate=faq&type=회원/탈퇴" className="myCate">
              회원/탈퇴
            </Link>
            <Link to="/qna?cate=faq&type=채팅" className="myCate">
              채팅
            </Link>
            <Link to="/qna?cate=faq&type=프로젝트" className="myCate">
              프로젝트
            </Link>
            <Link to="/qna?cate=faq&type=캘린더" className="myCate">
              캘린더
            </Link>
            <Link to="/qna?cate=faq&type=페이지" className="myCate">
              페이지
            </Link>
            <Link to="/qna?cate=faq&type=기타" className="myCate">
              기타
            </Link>
          </div>
          <br />
          ...
          <div>
            {serverData.dtoList.map((data, index) => {
              return (
                <>
                  <p className="qnaTitle" onClick={() => yameAcoHandler(index)}>
                    Q {data.title}
                  </p>
                  <p
                    className="qnaAnswer"
